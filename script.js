...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateTotals(subtotal) {
        ...
        if (payAmt) payAmt.textContent = '$' + finalTotal.toFixed(2);
    }
    ...
    // ==========================================================================
    // 10. CHECKOUT MODAL & ORDER PLACEMENT
    // ==========================================================================
    ...
    const scheduleInputs = document.getElementById('schedule-inputs-container');
    if (scheduleCheckbox && scheduleInputs) {
        scheduleCheckbox.addEventListener('change', function () {
            scheduleInputs.classList.toggle('show', this.checked);
        });
    }

    if (placeOrderBtn && checkoutModal) {
        var _placeOrderLock = false;
        placeOrderBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            if (_placeOrderLock) return;
            _placeOrderLock = true;
            setTimeout(function() { _placeOrderLock = false; }, 600);
            if (cart.length === 0) {
                showToast('Your tray is empty! Add some treats first.', 'error');
                return;
            }
            var totalEl = document.getElementById('tray-total-price');
            var total = totalEl ? totalEl.textContent : '$0.00';
            if (payAmountSpan) payAmountSpan.textContent = total;
            closeTray();
            setTimeout(function() {
                checkoutModal.style.display = 'flex';
                void checkoutModal.offsetHeight;
                checkoutModal.classList.add('active');
                var payBtn = document.getElementById('pay-now-btn');
                if (payBtn) payBtn.dataset.processing = '';
            }, 320);
        });
    }
    ...
    if (paymentForm) {
        paymentForm.addEventListener('submit', function (e) {
            ...
            if (payBtn.dataset.processing === 'true') return;
            payBtn.dataset.processing = 'true';

            const customerName = (document.getElementById('cc-name')?.value || '').trim() || 'Valued Customer';
            const originalHTML = payBtn.innerHTML;

            payBtn.innerHTML = 'Processing... ⌛';
            payBtn.style.cssText = 'background-color:#e67e22;pointer-events:none;width:100%;margin-top:10px;font-size:1rem;';
            ...
            setTimeout(() => {
                try {
                    ...
                    const totalText = document.getElementById('tray-total-price')?.textContent || '$0.00';
                    ...
                    const orderId = 'SD-' + Math.floor(Math.random() * 90000 + 10000);
                    const newOrder = {
                        id: orderId,
                        customerName: customerName,
                        customerEmail: (function() { try { return JSON.parse(localStorage.getItem('bakeryCurrentUser') || '{}').email || ''; } catch(e) { return ''; } })(),
                        date: orderDateText,
                    ...
                    // Fire-and-forget email — completely non-blocking with AbortController timeout
                    try {
                        ...
                        fetch('https://formspree.io/f/xvzwzlln', {
                            ...
                            signal: ctrl.signal,
                            body: JSON.stringify({ Subject: 'New Order #' + orderId, Customer: customerName, Total: '$' + grandTotal })
                        }).catch(function() {});
                    } catch(fetchErr) { /* silent - email optional */ }

                    // Save order to localStorage
                    try {
                        const orderHistory = safeStorage.getJSON('bakeryOrders', []);
                        orderHistory.unshift(newOrder);
                        safeStorage.setJSON('bakeryOrders', orderHistory);
                    } catch(storageErr) { console.warn('Storage error:', storageErr); }

                    // Loyalty points
                    try {
                        const currentUser = JSON.parse(localStorage.getItem('bakeryCurrentUser') || 'null');
                        if (currentUser && currentUser.email) {
                            ...
                            safeStorage.setJSON('bakeryCurrentUser', currentUser);
                            const users = JSON.parse(localStorage.getItem('bakeryUsers') || '[]');
                            const ui = users.findIndex(function(u) { return u.email === currentUser.email; });
                            if (ui >= 0) { users[ui].loyaltyPoints = currentUser.loyaltyPoints; safeStorage.setJSON('bakeryUsers', users); }
                            showToast('+' + pointsEarned + ' Loyalty Points earned! 🌟', 'success');
                        }
                    } catch(loyaltyErr) { /* silent */ }

                    // Clear cart
                    cart = [];
                    discountPercent = 0;
                    try { localStorage.setItem('bakeryCart', JSON.stringify([])); } catch(e) {}
                    renderCart();

                    // Success UI
                    payBtn.innerHTML = '🎉 Order Placed!';
                    payBtn.style.cssText = 'background-color:#27ae60;pointer-events:none;width:100%;margin-top:10px;font-size:1rem;';

                    // Confetti - safe guard
                    try {
                        if (typeof window.confetti === 'function') {
                            window.confetti({ particleCount: 150, spread: 75, origin: { y: 0.6 }, zIndex: 9999999 });
                        }
                    } catch(confettiErr) { /* silent */ }

                    // Close modal and redirect after delay
                    setTimeout(function() {
                        try {
                            if (checkoutModal) closeCheckoutModal();
                            payBtn.innerHTML = originalHTML;
                            payBtn.style.cssText = '';
                            payBtn.dataset.processing = '';
                            closeTray();
                            paymentForm.reset();
                            showToast('Order placed successfully! 🎂', 'success');
                        } catch(uiErr) { /* silent */ }

                        // GitHub Pages safe relative redirect
                        setTimeout(function() {
                            try {
    ...
    function renderOrders() {
        ...
        orderHistory.forEach(order => {
            let itemsHtml = (order.items || []).map(item =>
                `<li><span>${item.emoji || '🧁'} ${item.name}</span><span>${item.price}</span></li>`
            ).join('');
    ...
    if (closeQuickView) closeQuickView.addEventListener('click', () => quickViewModal?.classList.remove('active'));
    if (quickViewModal) quickViewModal.addEventListener('click', e => { if (e.target === quickViewModal) quickViewModal.classList.remove('active'); });
        ...
        overlay.innerHTML = `
            <div id="receipt-modal-card">
                ...
                <div id="receipt-modal-actions">
                    <button id="receipt-print-btn" class="btn" style="margin-top:0;padding:12px 28px;font-size:0.92rem;">🖨️ Print Receipt</button>
                </div>
            </div>`;
        ...
        document.getElementById('receipt-print-btn').addEventListener('click', () => {
            ...
            <style>
                ...
                *{box-sizing:border-box;margin:0;padding:0}
                body{font-family:'Outfit',sans-serif;background:#fdfbf7;padding:30px;color:#1e1208}
                .rct-box{border:2px solid #c8401e;border-radius:20px;padding:40px;max-width:480px;margin:auto}
                .rct-logo{text-align:center;font-size:3.5rem;margin-bottom:8px}
                .rct-title{text-align:center;color:#c8401e;font-size:1.4rem;letter-spacing:3px;text-transform:uppercase;font-weight:700;margin-bottom:4px}
                .rct-sub{text-align:center;color:#888;font-size:0.82rem;margin-bottom:20px}
                .rct-divider{border:none;border-top:2px dashed #f0e8e0;margin:18px 0}
                .rct-row{display:flex;justify-content:space-between;margin-bottom:7px;font-size:0.9rem}
                .rct-row strong{color:#1e1208}
                .rct-badge{background:#27ae60;color:white;padding:2px 10px;border-radius:20px;font-size:0.75rem;font-weight:700}
                .rct-items{width:100%;border-collapse:collapse;margin:10px 0}
                .rct-items td{padding:9px 0;border-bottom:1px solid #f0e8e0;font-size:0.9rem}
                .rct-items td:last-child{text-align:right;font-weight:700;color:#c8401e}
                .rct-total{text-align:right;font-size:1.6rem;font-weight:700;color:#c8401e;margin-top:14px}
                .rct-footer{text-align:center;margin-top:22px;font-size:0.8rem;color:#aaa;line-height:1.9;font-style:italic}
            </style></head><body>${body}<script>window.onload=()=>{setTimeout(()=>{window.print();window.close();},500)}<\/script></body></html>`);
            win.document.close();
        });
