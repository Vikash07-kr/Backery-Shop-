...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let discountPercent = 0; // BUG FIX: coupon discount properly tracked
    ...
    document.querySelectorAll('.save-wishlist-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            const wishlist = JSON.parse(localStorage.getItem('bakeryWishlist')) || [];
    ...
    function renderWishlist() {
        ...
        const savedItems = JSON.parse(localStorage.getItem('bakeryWishlist')) || [];
        ...
        savedItems.forEach((item, index) => {
            ...
            itemCard.querySelector('.remove-wishlist-btn').addEventListener('click', function () {
                ...
                localStorage.setItem('bakeryWishlist', JSON.stringify(savedItems));
    ...
    if (ordersContainer) {
        renderOrders();
        let lastOrderData = localStorage.getItem('bakeryOrders');
        setInterval(() => {
            const newData = localStorage.getItem('bakeryOrders');
    ...
    window.renderAdminOrders = function () {
        ...
        const allOrders = JSON.parse(localStorage.getItem('bakeryOrders')) || [];
    ...
    window.updateOrderStatus = function (orderIndex, newStatus) {
        ...
        localStorage.setItem('bakeryOrders', JSON.stringify(allOrders));
    ...
    if (orderForm) {
        orderForm.addEventListener('submit', function (e) {
            ...
            fetch('https://formspree.io/f/xvzwzlln', {
                ...
                orderForm.reset();
                showToast('Your cake request was sent! We\'ll be in touch 🎂', 'success');
                setTimeout(() => { submitBtn.classList.remove('success'); btnText.textContent = 'Send Request ✉️'; }, 3500);
            }).catch(() => {
                submitBtn.classList.remove('loading');
                btnText.textContent = 'Send Request ✉️';
    ...
    function renderPublicReviews() {
        ...
        if (!container) return;
        const allReviews = JSON.parse(localStorage.getItem('bakeryReviews')) || [];
    ...
    if (submitFeedback) {
        submitFeedback.onclick = () => {
            if (selectedRating > 0 && feedbackText?.value.trim()) {
                ...
                localStorage.setItem('bakeryReviews', JSON.stringify(allReviews));
    ...
    if (authForm) {
        authForm.addEventListener('submit', (e) => {
            ...
            if (isSignUp) {
                ...
                users.push(newUser);
                localStorage.setItem('bakeryUsers', JSON.stringify(users));
                localStorage.setItem('bakeryCurrentUser', JSON.stringify(newUser));
                ...
                localStorage.setItem('bakeryCurrentUser', JSON.stringify(user));
